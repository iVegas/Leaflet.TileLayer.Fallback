export function createTileLayerFallback(extend, TileLayer, Browser, Util) {
	TileLayer.Fallback = TileLayer.extend({
	  options: {
		minNativeZoom: 0
	  },
	  initialize: function (urlTemplate, options) {
		TileLayer.prototype.initialize.call(this, urlTemplate, options);
	  },
	...
	TileLayer.fallback = function (urlTemplate, options) {
	  return new TileLayer.Fallback(urlTemplate, options);
	};
  }
  
  export function createTileLayerFallbackCompact(L) {
	return createTileLayerFallback(L.extend, L.TileLayer, L.Browser, L.Util);
  }
  
  // Usage Example:
  // import { createTileLayerFallbackCompact } from './path-to-this-module';
  // import L from 'leaflet';
  // createTileLayerFallback(L);
